...
import { UPDATE_VIEW_LESSON_ROUTE, UPDATE_VIEW_LESSON_TOGGLE_ROUTE } from "../../shared/constant/url";
...
export default function LessonInScroll({
  ...
  const updateViewToggle = async () => {
    let resp = await apiPost(
      UPDATE_VIEW_LESSON_TOGGLE_ROUTE +
      `?coursShortId=${query.get("shortId")}&lessonId=${lesson._id}`
    );
    if (views.includes(lesson._id)) {
      ...
      <div className="d-flex align-items-center">
        <div>
          {views.includes(lesson._id) ? (
            <BsCheck2Square onClick={(e) => { updateViewToggle(); e.stopPropagation() }} size={17} style={{ marginRight: "-2.5px", color: "black" }} />
          ) : (
            <BsSquare onClick={(e) => { updateViewToggle(); e.stopPropagation() }} size={15} />
          )}
        </div>
